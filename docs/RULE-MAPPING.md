...
这是一份演进账本，不表示所有原始流程都按原义保留。当前版本将协作分为不启用、轻量协作和受控协作；协作等级由任务能否拆分，以及任务的重要性、复杂度、信息量和遗漏跑偏风险决定。Canary、工作目录和 FINAL 分别按调用风险、产物结构和交接需要触发，不参与协作分级。运行时事实源仍是 `SKILL.md`、对应 references、[行为契约](../skills/agent-cowork-control/references/behavior-contract.md) 和 T01–T14 回归测试。

| 原始规则 | 原始位置 | 当前处理 | 当前位置 | 测试 |
|---|---|---|---|---|
| 主代理负责、子代理无最终回复且不再委派 | `SKILL.md` 核心规则1 | 保留 | `SKILL.md` 核心规则1 | T02、T07 |
| 六段 Plan、独立审查与明确批准 | `SKILL.md` 规则2、Plan 双通道；`task-packets.md` | 分层：单步且不需拆分的任务无 Plan；普通多步骤和轻量协作使用简短“步骤 → 验证”计划并由用户确认；受控协作面向用户称为加强协作，保留六段 Plan、用户确认和一次独立 Plan 审查 | `SKILL.md` 规则2、加强协作开始前的两项确认；`task-packets.md` | T01、T12 |
| Agent 选择、默认模型继承、不可静默替换 | `SKILL.md` 规则3 | 参数化：用户指定优先，否则按职责选择；无合适角色时使用当前角色子 Agent | `SKILL.md` 规则3；`task-packets.md` 运行配置与派发信封 | T02 |
| Plan 审查使用 read、独立新 thread、结束后关闭 | `SKILL.md` Plan 双通道 | 仅受控协作使用；每个获批 Plan 最多一次 | `SKILL.md` 加强协作开始前的两项确认；`task-packets.md` Plan 审查包与账本 | T01、T02、T11 |
| 最小 read/write 权限 | `SKILL.md` 规则4；各协议 | 保留 | `SKILL.md` 规则5；各协议 | T03 |
| 每个部分一个执行者、补充查询不重复派人 | `SKILL.md` 规则5；`task-packets.md` | 单一责任保留；正式 `QUERY_BACKLOG` 仅用于受控协作 | `SKILL.md` 规则6；`task-packets.md` | T05 |
| 结构化工具先 Canary、同 thread 生产 | `SKILL.md` 规则6；`execution-protocol.md` | 条件化：已知、单次、只读且结果可直接核对的调用可直接执行；批量、写入、不确定、高成本或高影响调用在任一协作等级都先 Canary，调用风险不改变协作等级 | `SKILL.md` 规则7；`execution-protocol.md` | T04 |
| 证据外部化、确定性断言、可重跑 | `SKILL.md` 规则7；执行、研究与验证协议 | 保留，按任务规模使用 | `SKILL.md` 规则8；完整角色协议 | T06、T07 |
| `runs/...` 隔离、单目录单写者 | `SKILL.md` 规则8；`task-packets.md` | 条件化：普通单一写入者使用授权目标目录；任何协作等级需要多人写入、批量或正式数据、可重跑证据或中间文件隔离时，都使用授权 `run_root`，目录需求不改变协作等级 | `SKILL.md` 规则9；`task-packets.md` | T06 |
| FINAL 字段、stage 不等于验收 | `SKILL.md` 规则9；`task-packets.md` | 条件化：任何协作等级仅在需要后续接手、跨会话恢复、多产物汇总或用户明确要求正式证据包时创建 `FINAL.md`，交接需求不改变协作等级 | `SKILL.md` 规则10；`task-packets.md` FINAL 模板 | T07 |
...
- 先判断任务能否拆出独立部分，再按重要性、复杂度、信息量和遗漏跑偏风险选择轻量或受控协作。
- 资料、步骤或文件较多可以成为分工理由，但不会因数量本身自动启用协作。
- 将六段 Plan、独立 Plan 审查、严谨交接和独立结果审核限定于受控协作。
- Canary、运行目录和 `FINAL.md` 分别按调用风险、产物结构和交接需要触发，不自动改变协作等级。
- 正式数据、权限、外部系统和高风险操作继续遵守安全规则，但不再用于定义协作等级。
- 将固定审批者、Agent、工作区和金融领域措辞参数化为公开可复用的配置。

## 始终保留的安全边界

- 研究、检查和审查默认只读；写权限只授予明确写入责任人。
- 每个部分只有一个执行者，不允许写入范围重叠或归属不明。
