...
任务涉及多个写入责任域、批量或正式数据落盘、独立可重跑证据，或中间产物需要与最终交付隔离时使用运行目录；不同写入责任域使用互不重叠的目录。单一写入者在 clean、明确授权的 Git 工作区或用户指定输出目录内完成普通源码、文档、报告、演示文稿、表格或网页时跳过本节。只读研究和直接对话回答也跳过本节。
...
仅任务需要持久交接、跨会话恢复、多产物汇总，或用户明确要求正式证据包时使用。运行目录存在本身不触发 FINAL；由主代理在总任务根目录或用户指定的正式交付根目录维护唯一文件，子代理不分别创建。
