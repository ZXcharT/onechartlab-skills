...
```markdown
| 负责部分/交付物 | 唯一责任人 | 输入与前置依赖 | 产出路径 | 交接对象 | 禁止重叠范围 | 失败后接管者 |
|---|---|---|---|---|---|---|
```

## 运行隔离控制（按条件）

任何协作等级中，只要任务需要多个助手分别写入不同部分、批量或正式数据落盘、独立可重跑证据，或把中间文件与最终交付分开，就使用运行目录；不同写入部分使用互不重叠的目录。单一写入者在 clean、明确授权的 Git 工作区或用户指定输出目录内完成普通源码、文档、报告、演示文稿、表格或网页时跳过本节。只读研究和直接对话回答也跳过本节。
...
任何协作等级中，只有任务以后可能需要继续、需要跨会话恢复、需要汇总多个产物，或用户明确要求正式证据包时才使用。运行目录存在本身不触发 FINAL；由主代理在总任务根目录或用户指定的正式交付根目录维护唯一文件，子代理不分别创建。
...
任务使用外部数据、指标口径或多来源证据时，必须增加“关键口径与数据源”；发生过会影响可信度、复现、恢复或维护的重要异常时，必须增加“重大异常与恢复”。FINAL 只记录证据和状态，不替代真实验证、用户批准或外部发布授权。

## QUERY_BACKLOG

```markdown
| 查询问题 | 所属部分 | 用途/为何需要 | 紧急度 | 建议责任人 |
|---|---|---|---|---|
```

已有部分优先续开放的原 `threadId`；出现新的独立方向时才另派 read 研究者。主代理必须处理关键查询时，记录“为何关键、查什么、做到哪里停止”。
...
新建 thread 用于独立角色；Canary 通过后和原负责部分的补充任务可续原 thread。续派沿用已绑定身份，不跨角色复用 thread。Plan 审查 thread 关闭后，后续研究、执行、验证必须新建 thread。
